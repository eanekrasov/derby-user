  ...
  return function () {
    ...
    app.post('/user/changeEmail', function (req, res) {
      ...
      model.fetch($private, $query, function (err) {
        ...
        credential.hash(token, function (err, hashedToken) {
          if (err) return res.send(500, {error: err});
          var oldEmail = $private.get('local.email.value');
          $private.set('local.email.token.date', new Date());
          $private.set('local.email.token.hash', hashedToken);
          $private.set('local.email.value', email);
          $private.set('local.email.verified', false, function () {
    ...
    app.post('/user/changeUsername', function (req, res) {
      var model = req.getModel()
        ...
        , userId = model.get('_session.user.id')
        , $query = model.query('users', {id: {$ne: userId}, 'local.username': username})
        , $public = model.at('users.' + userId);
    ...
    app.get('/user/:id/confirmEmail/:token', function (req, res, next) {
      ...
      $private.fetch(function (err) {
        ...
        var elapsed = moment().diff($private.get('local.email.token.date'));
        if (elapsed > options.tokenDuration) return next('token expired');
        var hashedToken = $private.get('local.email.token.hash');
        credential.verify(hashedToken, token, function (err, valid) {
          if (err) return next(err);
          if (!valid) return next('invalid token');
          $private.del('local.email.token');
          $private.set('local.email.verified', true, function () {
    ...
    app.post('/user/signout', function (req, res) {
      ...
      $public.fetch(function (err) {
        ...
        req.logout();
        req.session.user = {id: userId};
        model.add('users', {created: new Date(), id: userId, isRegistered: false});
        model.add('auth', {id: userId});
    ...
    app.post('/user/signup', function (req, res) {
      var model = req.getModel()
        ...
        , $public = model.at('users.' + userId)
        , $private = model.at('auth.' + userId)
        , $query1 = model.query('users', {'local.username': username})
        , $query2 = model.query('auth', {'local.email.value': email});
        ...
        function (err) {
          ...
          credential.hash(token, function (err, hashedToken) {
            ...
            credential.hash(password, function (err, hashedPassword) {
              ...
              $private.set('local.email.token.hash', hashedToken);
              $private.set('local.email.value', email);
              $private.set('local.email.verified', false);
    ...
    app.post('/user/verifyEmail', function (req, res) {
      var model = req.getModel()
        , email = (req.body.email || '').trim()
        , userId = model.get('_session.user.id')
        , $query = model.query('auth', {id: {$ne: userId}, 'local.email.value': email})
        , $private = model.at('auth.' + userId);
      ...
      model.fetch($private, $query, function (err) {
        ...
        if (email) $private.set('local.email.value', email);
        var token = model.id();
        credential.hash(token, function (err, hashedToken) {
          if (err) return res.send(500, {error: err});
          $private.set('local.email.token.date', new Date());
          $private.set('local.email.token.hash', hashedToken, function () {
